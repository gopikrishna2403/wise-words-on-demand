...
class Quote:
    ...
    def get_random_quote(cls, cursor):
        ...
        if use_azure_sql:
            ...
            cursor.execute(
                "SELECT TOP 1 id, quote_text, author, category, created_at FROM quotes ORDER BY NEWID()"
            )
        else:
            # SQLite uses RANDOM()
            cursor.execute(
                "SELECT id, quote_text, author, category, created_at FROM quotes ORDER BY RANDOM() LIMIT 1"
            )
    ...
    def get_all_quotes(cls, cursor):
        ...
        cursor.execute(
            "SELECT id, quote_text, author, category, created_at FROM quotes ORDER BY created_at DESC"
        )
    ...
    def get_stats(cls, cursor):
        ...
        return {"total_quotes": total_quotes, "total_authors": total_authors}
